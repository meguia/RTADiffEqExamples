...
using RealTimeAudioDiffEq, PlutoUI, Distributions

# ╔═╡ fd7e10e5-1604-4af6-9657-061bf71dc773
nosc = 8
...
# ╔═╡ 1b8fb37b-87f2-4bad-8085-e30dd09c8c00
@bind start Button("START")

# ╔═╡ 4e4af9c1-4a7f-47a4-bf58-dbf8858e1d56
@bind stop Button("STOP")

# ╔═╡ 2a8ed590-351b-46e5-8ada-dd80b913dbd4
begin
	k = rand(Gamma(5, 0.4),nosc)
	v0 = rand(Gamma(5, 0.1),nosc)
	u0 = repeat([0.1,0.],nosc)
	p = vcat([0.0],[[3*v0[n]^2-0.1,k[n],v0[n]] for n=1:nosc]...)
end;

# ╔═╡ 46a395c6-2cd0-42c8-b4e5-d0d0f71638e3
source = DESource(nreed!, u0, p; channel_map = [collect(1:4:2*nosc),collect(3:4:2*nosc)])

# ╔═╡ f6c9c1f8-a9ca-43ff-941f-e77b30654bfd
begin
	set_ts!(source,1500.0)
	set_gain!(source,0.2)
end	

# ╔═╡ 55772a16-44a8-4f83-83a6-0f8c94084d0f
let 
	start
...
# ╔═╡ ffcdc2cf-ae09-4116-ba63-ab29447c41f4
let 
	stop
...
# ╔═╡ 57d8b5c9-b5f9-4f97-98c2-41ddaf182989
begin
	gg = zeros(2,2*nosc)
	gg[1,1:2:2*nosc] = range(0, 1, nosc)
	gg[2,1:2:2*nosc] = range(1, 0, nosc)
	gg
end	

# ╔═╡ 80a0a2ae-0f79-4b95-9071-c775e5967491
length(range(0, 1, nosc))

# ╔═╡ efc7afd3-c113-4910-949f-2ced2bec0646
sp = html"&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp";

# ╔═╡ 8b2ae0c4-2b0d-44ba-89ab-e4bb9686bd87
md"""
dμ : $(@bind dμ Slider(-0.1:0.001:0.3,default=-0.1;show_value=true)) $sp
k0 : $(@bind k0 Slider(0.1:0.1:5.0,default=1.0;show_value=true)) $sp
γ : $(@bind γ Slider(0.0:0.01:1.0,default=0.0;show_value=true)) $sp
"""

# ╔═╡ e127b240-f7e4-48f4-aeb9-7ff04c23e1e2
for n=1:nosc
	set_param!(source,3*n-1,3.0*v0[n]^2+dμ)
end

# ╔═╡ 3f397309-fd1f-41e6-b95f-777c265b98f0
begin
	kv = rand(Gamma(5, k0/4.0),nosc)
	for n=1:nosc
		set_param!(source,3*n,kv[n])
	end
end	

# ╔═╡ 861e2ed9-ceba-4971-990e-190f67fe00da
set_param!(source,1,γ)
...
RealTimeAudioDiffEq = "7a3f4937-0cad-4220-a144-0330815b2118"

[compat]
Distributions = "~0.25.120"
PlutoUI = "~0.7.71"
RealTimeAudioDiffEq = "~0.1.0"
"""
...
manifest_format = "2.0"
project_hash = "c294a018b3f89df914c45416bf5e2c5ccb5eaf43"
...
[[deps.RealTimeAudioDiffEq]]
deps = ["Atomix", "DifferentialEquations", "libportaudio_jll"]
git-tree-sha1 = "9a46563f42070950397a4f0e563ae9d43659a9b9"
uuid = "7a3f4937-0cad-4220-a144-0330815b2118"
version = "0.1.0"
