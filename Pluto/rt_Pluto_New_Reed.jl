...
using Atomix,PortAudio.LibPortAudio, PlutoUI, Plots, DifferentialEquations
...
function nreed!(du,u,p,t)
    (μ,k,σ) = p
    du[1] = u[2] 
	du[2] = u[2]*(μ-u[2]*u[2])-k*u[1]*(1.0+σ*sqrt(k)*u[1]*u[1])
end

# ╔═╡ 46a395c6-2cd0-42c8-b4e5-d0d0f71638e3
source = DESource(nreed!, [0.1, 0.], [-0.1, 1.0,0.0]; channel_map = [2,2])
...
buttons = PlutoUI.ExperimentalLayout.Div([start_button,stop_button,reset_button], style=Dict(	"display" => "flex","flex-direction" => "row","background" => "gray"));
...
plot_phase = 
	plot(sol,idxs=(0,2),c=:yellow,label="",border=:none,ylims=(-2.5,2.5),size=(600,200));
...
par_widget = @bind par PlutoUI.combine() do Child
	md"""
	# New Reed
	μ : $(Child("μ", Slider(-0.01:0.005:3.0,default=-0.01;show_value=true))) $sp
	k : $(Child("k", Slider(0.1:0.002:2.0,default=0.2;show_value=true))) \
	σ : $(Child("σ", Slider(0.0:0.01:1.5,default=0.0;show_value=true))) 
	"""
end;
...
[[deps.Libtool_jll]]
deps = ["Artifacts", "JLLWrappers", "Libdl"]
git-tree-sha1 = "4375a578ffafebe499a52e26ca2d347a95cef57b"
