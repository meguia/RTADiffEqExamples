...
# ╔═╡ 64aa816e-4fe9-472f-b54b-206351fd37b1
using Plots
...
μ : $(@bind μ Slider(-6:0.01:2,default=-5.5;show_value=true))\
σ : $(@bind σ Slider(-2.0:0.1:1.0,default=-1.5;show_value=true)) \
ν : $(@bind ν Slider(0.01:0.01:0.2,default=0.1;show_value=true)) 
"""

# ╔═╡ 2002261d-1d74-43d7-86dc-84aa918bd931
begin
	dmu = 0.01
	muSN=-0.25*σ^2/ν
	muini = 1.2*muSN
	mu1=muini:dmu:0
	mu1b=dmu:dmu:2.0
	mu2=muSN+0.001:dmu:0
	mu3=muSN+0.001:dmu:2.0
	mu0=0:dmu:2.0
	plot(mu1,0 .* mu1,c=:blue,label="Reposo Estable")
	scatter!([μ],[sqrt(sum(source.data.state.u.^2))],ms=4,c=:blue,label="")
	plot!(mu1b,0 .* mu1b,c=:blue,ls=:dash,label="Reposo Inestable")
	if (σ<0)
		plot!(mu2,sqrt.(0.5*(-σ .- sqrt.(σ^2 .+ 4*mu2*ν))/ν),c=:black,ls=:dash,label="Umbral")
		plot!(mu3,sqrt.(0.5*(-σ .+ sqrt.(σ^2 .+ 4*mu3*ν))/ν),c=:red,label="Oscilacion Estable")
		plot!([0,0],[0,sqrt(-σ/ν)],arrow=true,color=:purple,la=0.2,lw=2,label="")
		plot!([muSN,muSN],[sqrt(-0.5*σ/ν),0],arrow=true,color=:purple,la=0.2,lw=2,label="")
	else
		plot!(mu0,sqrt.(0.5*(-σ .+ sqrt.(σ^2 .+ 4*mu0*ν))/ν),c=:red,label="Oscilacion Estable")
	end	
end	
...
begin 
	...
	@atomic source.data.control.p = [μ, σ, ν]
...
[[deps.Libtool_jll]]
deps = ["Artifacts", "JLLWrappers", "Libdl"]
git-tree-sha1 = "9e203cfcfb41d49338cbc4134452c40e7f4c93f6"
